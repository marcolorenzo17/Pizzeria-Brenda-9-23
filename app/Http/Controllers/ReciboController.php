...
class ReciboController extends Controller
{
    public function __invoke() {
        ...
        $recibos = Recibo::where('created_at', '>', Carbon::now()->subMonths(3))->where('idUser', '=', $user->id)->orderBy('id', 'desc')->paginate(10);
        return view('recibos', ['recibos' => $recibos]);
    }

    public function todosRecibos() {
        $user = User::findOrFail(Auth::user()->id);
        $recibos = Recibo::where('idUser', '=', $user->id)->orderBy('id', 'desc')->paginate(10);
        return view('recibos', ['recibos' => $recibos]);
    }

    public function recibosIndexAdmin() {
        $recibos = Recibo::where('created_at', '>', Carbon::now()->subMonths(3))->orderBy('id', 'desc')->paginate(10);
        return view('recibos', ['recibos' => $recibos]);
    }
